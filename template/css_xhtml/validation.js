...
function clearWraperErrorLabelsCSS(wraper) {
    // set all labels back to the normal class
    var i,
...
function _findWWCtrlNode(enclosingDiv) {
   var elems = enclosingDiv.getElementsByTagName("div");
   for(i = 0; i < elems.length; ++i ) {
       if (elems[i].className && elems[i].className.match(/(wwctrl)/)) {
           return elems[i];
       }
   }
...
function addErrorCSS(e, errorText) {
    try {
		...
		e.onchange = function(e){clearWraperErrorLabelsCSS(enclosingDiv)};
		e.onKeyUp = function(e){clearWraperErrorLabelsCSS(enclosingDiv)};
		e.attachEvent("onKeyUp", function(e){clearWraperErrorLabelsCSS(enclosingDiv)});
