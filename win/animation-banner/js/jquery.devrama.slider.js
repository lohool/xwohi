...
(function($){
	var DrSlider = function(element, options){
		...
		this.options = {
			...
			transition: 'random', //random, slide-left, slide-right, slide-top, slide-bottom, fade, split, split3d, door, wave-left, wave-right, wave-top, wave-bottom
			showProgress: true,
	...
	DrSlider.prototype = {
		...
		_prepare: function(callback){
			...
			this.$ele_projector.find('[data-lazy-url]').each(function(){
				$(this).bind("click",function(){
						document.location=$(this).data('lazy-url');
				});
			});
		...
		_attachNavigation: function(){
			...
			for(var i =0; i < this.num_slides; i++)
				...
				navigation_html += '<span class="nav-link index'+i+'" data-num="'+i+'">¡ñ</span>';
					
			this.$ele_projector.after('<div class="navigation devrama-slider">'+navigation_html+'</div>');
