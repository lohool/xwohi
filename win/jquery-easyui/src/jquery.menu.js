...
//			var m = $(e.target).closest('div.menu', allMenu);
		...
		function createMenu(menu){
			...
			if (menu.hasClass('menu-content')){
				...
				menu.children('div').each(function(){
					var item = $(this);
					var itemOpts = $.extend({}, $.parser.parseOptions(this,['name','iconCls','icon','href',{separator:'boolean'}]), {
					...
					if (!item.hasClass('menu-sep')){
						...
						var icon="";
				if (itemOpts.icon)icon="<img src='"+itemOpts.icon+"'>";
						if (itemOpts.iconCls){
							$('<div class="menu-icon">'+icon+'</div>').addClass(itemOpts.iconCls).appendTo(item);
						}
...
//			}
	...
	function appendItem(target, param){
		...
		var icon="";
				if (param.icon)icon="<img src='"+param.icon+"'>";
		if (param.iconCls) $('<div class="menu-icon">'+icon+'</div>').addClass(param.iconCls).appendTo(item);
	...
	$.fn.menu.methods = {
		...
		setIcon: function(jq, param){
			return jq.each(function(){
				...
				if (param.icon)icon="<img src='"+param.icon+"'>";
				if (param.iconCls){
					$('<div class="menu-icon">'+icon+'</div>').addClass(param.iconCls).appendTo(param.target);
					//alert('<div class="menu-icon">'+icon+'</div>')
				}
			});
		},
		/**
